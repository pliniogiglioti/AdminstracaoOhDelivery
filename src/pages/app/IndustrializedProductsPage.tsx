...
export function IndustrializedProductsPage() {
  ...
  function startEdit(p: IndustrializedProduct) {
    setEditingId(p.id)
    setForm({
      name: p.name,
      brand: p.brand ?? '',
  ...
  return (
    <div className="space-y-5">
      ...
      <form onSubmit={handleSubmit} className="panel-card grid gap-4 p-5 lg:grid-cols-[1fr_160px_180px_160px_80px_auto]">
        <label>
          <span className="text-xs font-bold uppercase tracking-[0.12em] text-ink-400">Nome</span>
          <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} required
            className="mt-2 h-11 w-full rounded-2xl border border-ink-100 px-3 text-sm outline-none focus:border-coral-300" placeholder="Coca-Cola Lata 350ml" />
        </label>
        <label>
          <span className="text-xs font-bold uppercase tracking-[0.12em] text-ink-400">Marca</span>
          <input value={form.brand} onChange={(e) => setForm((f) => ({ ...f, brand: e.target.value }))}
            className="mt-2 h-11 w-full rounded-2xl border border-ink-100 px-3 text-sm outline-none focus:border-coral-300" placeholder="Coca-Cola" />
        </label>
        <label>
          ...
          <input value={form.ean} onChange={(e) => setForm((f) => ({ ...f, ean: e.target.value }))}
            className="mt-2 h-11 w-full rounded-2xl border border-ink-100 px-3 text-sm outline-none focus:border-coral-300" placeholder="7894900011517" />
        </label>
        <label>
            ...
            <tbody className="divide-y divide-ink-100">
              {loading ? (
                ...
                <tr key={p.id}>
                  ...
                  <td className="px-4 py-4 text-ink-500 max-w-xs truncate">{p.description ?? '—'}</td>
